...
exports.default = (bot, logger) => {
    bot.onText(new RegExp('^/welcome+(?:@' + config_1.default.botinfo.username + ')? ([^\r]+)$'), (msg, match) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (msg.chat.type === 'private') {
                    ...
                    if (!isAdmin) {
                        ...
                        let value = yield model.message.findWelcome(chatid);
                        if (!value) {
                            yield model.message.createWelcome(chatid, match[1]);
                            yield bot.sendMessage(chatid, ' ' + temp.text('command.welcome.success'), {
                            ...
                            if (value && !value.welcomeMessage) {
                                ...
                                yield model.message.updateWelcome(chatid, match[1]);
                                yield bot.sendMessage(chatid, ' ' + temp.text('command.welcome.success'), {
                                    reply_to_message_id: msg.message_id
                                });
                                logger.info('chatid: ' + chatid + ', username: ' + helper_1.default.getuser(msg.from) + ', lang: ' + msg.from.language_code + ', command: ' + msg.text + ', type: update success');
    ...
    bot.onText(new RegExp('^/welcome+(?:@' + global.botinfo.username + ')? ?$'), (msg, match) => __awaiter(this, void 0, void 0, function* () {
        if (Math.round((new Date()).getTime() / 1000) - msg.date <= 180) {
            const chatid = msg.chat.id;
            let temp;
            try {
                ...
                if (msg.chat.type !== 'group' && msg.chat.type !== 'supergroup') {
                    ...
                    yield bot.sendMessage(chatid, '🔧 ' + temp.text('command.welcome.help'), {
                        reply_to_message_id: msg.message_id,
                        parse_mode: 'Markdown'
                    });
