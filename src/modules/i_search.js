        ...
        if(match) {
            ...
            try {
                ...
                if(typeof match[2] == 'undefined' || match[2] == '') {
                    try {
                        await bot.answerInlineQuery(q.id, [{type: 'article', title: '@'+global.botinfo.username+' (search|google|query) (keyword)', id: 'help', input_message_content: {
                            ...
                            try {
                                ...
                                try {
                                    await bot.answerInlineQuery(q.id, [{type: 'article', title: 'error', id: 'error', input_message_content: {
                                            ...
                                            .replace(/{botid}/g, '@'+global.botinfo.username).replace(/{keyword}/g, match[2]), parse_mode: 'HTML', disable_web_page_preview: true
                                            }}], {cache_time: 3});
                                    logger.error('inlineid: '+q.id+', username: '+helper.getuser(msg.from)+', lang: '+msg.from.language_code+', command: '+msg.query+', type: error');
                                    logger.debug(e.stack);
                                } catch(e) {
